 ...
 * evaluating a product-1, yielding another product-1 from which the result of
 * its evaluation can be retrieved at a later time.
 * <p/>
 * 
 * @version %build.number%<br>
 *          <ul>
 *          <li>$LastChangedRevision$</li> <li>$LastChangedDate:
 *          2009-05-09 10:46:32 -0500 (Sat, 09 May 2009) $</li> <li>Author:
 *          runar</li>
 *          </ul>
 */
public final class Strategy<A> {
  ...
  /**
   * Returns the functional representation of this Strategy, a function that
   * evaluates a product-1.
   * 
   * @return The function representing this strategy, which evaluates a
   *         product-1.
   */
   ...
   * @return A function that executes concurrently when called, yielding a
   *         Future value.
   */
   ...
   * @return A function that executes concurrently when called, yielding a
   *         product-1 that returns the value.
   */
  ...
  /**
   * Maps the given function over the given list in parallel using this
   * strategy.
   * 
   * @param f
   *          A function to map over the given list in parallel.
   * @param bs
   *          A list over which to map the given function in parallel.
   * @return A product-1 that returns the list with all of its elements
  ...
  /**
   * Maps the given function over the given array in parallel using this
   * strategy.
   * 
   * @param f
   *          A function to map over the given array in parallel.
   * @param bs
   *          An array over which to map the given function in parallel.
   * @return A product-1 that returns the array with all of its elements
   *         transformed by the given function.
   */
  ...
  /**
   * A strict version of parMap over lists. Maps the given function over the
   * given list in parallel using this strategy, blocking the current thread
   * until all values have been obtained.
   * 
   * @param f
   *          A function to map over the given list in parallel.
   * @param bs
  ...
  /**
   * A strict version of parMap over arrays. Maps the given function over the
   * given arrays in parallel using this strategy, blocking the current thread
   * until all values have been obtained.
   * 
   * @param f
   *          A function to map over the given array in parallel.
   * @param bs
   *          An array over which to map the given function in parallel.
   * @return An array with all of its elements transformed by the given
   *         function.
   */
   ...
   * 
   * @return A function that promotes another function to a parallel function on
   *         lists.
   */
   ...
   * 
   * @return A function that promotes another function to a blocking parallel
   *         function on lists.
   */
  ...
  /**
   * Binds the given function in parallel across the given list, using the given
   * strategy, with a final join.
   * 
   * @param s
   *          The strategy to use for parallelization.
   * @param f
   *          The function to bind across the given list.
   * @param as
   *          The list across which to bind the given function.
   * @return A P1 containing the result of the parallel map operation after the
   *         final join.
   */
  public static <A, B> P1<List<B>> parFlatMap(final Strategy<List<B>> s,
      final F<A, List<B>> f, final List<A> as) {
    return fmap(List.<B> join()).f(s.parMap(f, as));
  }

  /**
   * Binds the given function in parallel across the given array, using the
   * given strategy, with a final join.
   * 
   * @param s
   *          The strategy to use for parallelization.
   * @param f
   *          The function to bind across the given array.
   * @param as
  ...
  public static <A, B> P1<Array<B>> parFlatMap(final Strategy<Array<B>> s,
      final F<A, Array<B>> f, final Array<A> as) {
    return fmap(Array.<B> join()).f(s.parMap(f, as));
  }

  /**
   * Sequentially evaluates chunks (sub-sequences) of a list in parallel. Splits
   * the list into chunks, evaluating the chunks simultaneously, but each chunk
   * as a sequence.
   * 
   * @param s
   *          The strategy to use for parallelization.
   ...
   * @return A product-1 containing the list of results extracted from the given
   *         list of product-1s.
   */
  public static <A> P1<List<A>> parListChunk(final Strategy<List<A>> s,
      final int chunkLength, final List<P1<A>> as) {
    return fmap(List.<A> join()).f(
        s.parList(as.partition(chunkLength).map(P1.<A> sequenceList())));
  }

  /**
   * Zips together two lists in parallel using a given function, with this
   * strategy. Calls the given function once for each corresponding pair in the
   * lists, position-wise, passing elements from the first list to the first
   * argument of the function, and elements from the second list to the second
   * argument of the function, yielding a list of the results. If the lists are
   * not of the same length, the remaining elements of the longer list are
   * ignored.
   * 
   * @param f
   ...
   * @param cs
   *          A list to zip with the given function.
   * @return The list of the results of calling the given function on
   *         corresponding elements of the given lists.
   */
  public <B, C> P1<List<A>> parZipWith(final F2<B, C, A> f, final List<B> bs,
      final List<C> cs) {
    return sequence(bs.zipWith(cs, concurry(f)));
  }

  /**
   * Zips together two arrays in parallel using a given function, with this
   * strategy. Calls the given function once for each corresponding pair in the
   * arrays, position-wise, passing elements from the first array to the first
   * argument of the function, and elements from the second array to the second
   * argument of the function, yielding a array of the results. If the arrays
   * are not of the same length, the remaining elements of the longer array are
   * ignored.
   * 
   * @param f
   *          The function of arity-2 with which to zip.
   ...
   * @param cs
   *          A array to zip with the given function.
   * @return The array of the results of calling the given function on
   *         corresponding elements of the given arrays.
   */
  public <B, C> P1<Array<A>> parZipWith(final F2<B, C, A> f, final Array<B> bs,
      final Array<C> cs) {
    return sequence(bs.zipWith(cs, concurry(f)));
  }

  /**
   * Lifts a given function of arity-2 so that it zips together two lists in
   * parallel, using this strategy, calling the function once for each
   * corresponding pair in the lists, position-wise.
   * 
   * @param f
   *          The function of arity-2 with which to zip.
   * @return A transformation that zips two lists using the argument function,
   *         in parallel.
   */
  public <B, C> F2<List<B>, List<C>, P1<List<A>>> parZipListWith(
      final F2<B, C, A> f) {
  ...
  /**
   * Lifts a given function of arity-2 so that it zips together two arrays in
   * parallel, using this strategy, calling the function once for each
   * corresponding pair in the arrays, position-wise.
   * 
   * @param f
   *          The function of arity-2 with which to zip.
   * @return A transformation that zips two arrays using the argument function,
   *         in parallel.
   */
  public <B, C> F2<Array<B>, Array<C>, P1<Array<A>>> parZipArrayWith(
      final F2<B, C, A> f) {
  ...
  /**
   * Provides a strategy that performs sequential (non-concurrent) evaluation of
   * its argument.
   * 
   * @return A strategy that performs sequential (non-concurrent) evaluation of
   *         its argument.
   */
  ...
  /**
   * Maps the given bijective transformation across this strategy (Exponential
   * Functor pattern).
   * 
   * @param f
   *          A transformation from this strategy's codomain to the resulting
   *          strategy's codomain.
   * @param g
   *          A transformation from the resulting strategy's domain to this
   *          strategy's domain.
  ...
  /**
   * Maps the given transformation across this strategy's domain (Invariant
   * Functor pattern).
   * 
   * @param f
   *          A transformation from this strategy's codomain to the resulting
   *          strategy's codomain.
   * @return A new strategy that applies the given transformation after each
  ...
  /**
   * Maps the given transformation across this strategy's codomain (Invariant
   * Functor pattern).
   * 
   * @param f
   *          A transformation from the resulting strategy's domain to this
   *          strategy's domain.
   * @return A new strategy that applies the given transformation before each
   *         application of this strategy.
   */
  ...
  /**
   * Provides an error-handling strategy. Captures any uncaught runtime errors
   * encountered by this strategy and applies the given side-effect to them.
   * 
   * @param e
  ...
  /**
   * Provides an error-handling strategy. Captures any uncaught runtime errors
   * encountered by the given strategy and applies the given side-effect to
   * them.
   * 
   * @param s
   *          The strategy to equip with an error-handling effect.
   * @param e
   *          The effect that should handle errors.
   * @return A strategy that captures any runtime errors with a side-effect.
   */
  public static <A> Strategy<A> errorStrategy(final Strategy<A> s,
      final Effect<Error> e) {
