...
/**
 * Provides a pointed stream, which is a non-empty stream structure that tracks an index (focus)
...
public class StreamZipper<A> {
  ...
  private StreamZipper(final Stream<A> left, final A focus, final Stream<A> right) {
    this.left = left;
    this.focus = focus;
    this.right = right;
  }
   ...
   */
  public static <A> StreamZipper<A> streamZipper(final Stream<A> left, final A focus, final Stream<A> right) {
    return new StreamZipper<A>(left, focus, right);
  }
   ...
   */
  public static <A> StreamZipper<A> streamZipper(final P3<Stream<A>, A, Stream<A>> p) {
    return new StreamZipper<A>(p._1(), p._2(), p._3());
  }
  ...
  public <B> StreamZipper<B> map(final F<A, B> f) {
    return streamZipper(left.map(f), f.f(focus), right.map(f));
  }
  ...
  public static <A> Option<StreamZipper<A>> fromStreamEnd(final Stream<A> a) {
    if (a.isEmpty())
      ...
      return some(streamZipper(xs.tail()._1(), xs.head(), Stream.<A>nil()));
    }
  }
  ...
  public Option<StreamZipper<A>> next() {
    return right.isEmpty() ? Option.<StreamZipper<A>>none() : some(tryNext());
  }
  ...
  public StreamZipper<A> tryNext() {
    if (right.isEmpty())
      ...
      return streamZipper(left.cons(focus), right.head(), right.tail()._1());
  }
  ...
  public Option<StreamZipper<A>> previous() {
    return right.isEmpty() ? Option.<StreamZipper<A>>none() : some(tryPrevious());
  }
  ...
  public StreamZipper<A> tryPrevious() {
    if (left.isEmpty())
      ...
      return streamZipper(left.tail()._1(), left.head(), right.cons(focus));
  }
   ...
   */
  public static <A> F<StreamZipper<A>, Option<StreamZipper<A>>> next_() {
    return new F<StreamZipper<A>, Option<StreamZipper<A>>>() {
      public Option<StreamZipper<A>> f(final StreamZipper<A> as) {
        return as.next();
      }
    };
  }
   ...
   */
  public static <A> F<StreamZipper<A>, Option<StreamZipper<A>>> previous_() {
    return new F<StreamZipper<A>, Option<StreamZipper<A>>>() {
      public Option<StreamZipper<A>> f(final StreamZipper<A> as) {
        return as.previous();
      }
   ...
   */
  public StreamZipper<A> insertLeft(final A a) {
    return streamZipper(left, a, right.cons(focus));
  }
   ...
   */
  public StreamZipper<A> insertRight(final A a) {
    return streamZipper(left.cons(focus), a, right);
  }
   ...
   */
  public Option<StreamZipper<A>> deleteLeft() {
    return left.isEmpty() && right.isEmpty()
        ? Option.<StreamZipper<A>>none()
        : some(streamZipper(left, left.isEmpty() ? right.head() : left.head(), right));
  }
   ...
   *         would cause the zipper to be empty.
   */
  public Option<StreamZipper<A>> deleteRight() {
    return left.isEmpty() && right.isEmpty()
        ? Option.<StreamZipper<A>>none()
        : some(streamZipper(left, right.isEmpty() ? left.head() : right.head(), right));
  }
   ...
   */
  public StreamZipper<A> deleteOthers() {
    final Stream<A> nil = nil();
    return streamZipper(nil, focus, nil);
  }
   ...
   */
  public StreamZipper<StreamZipper<A>> positions() {
    final Stream<StreamZipper<A>> left = Stream.unfold(
        new F<StreamZipper<A>, Option<P2<StreamZipper<A>, StreamZipper<A>>>>() {
          public Option<P2<StreamZipper<A>, StreamZipper<A>>> f(final StreamZipper<A> p) {
            return p.previous().map(join(P.<StreamZipper<A>, StreamZipper<A>>p2()));
          }
        }, this);
    final Stream<StreamZipper<A>> right = Stream.unfold(
        new F<StreamZipper<A>, Option<P2<StreamZipper<A>, StreamZipper<A>>>>() {
          public Option<P2<StreamZipper<A>, StreamZipper<A>>> f(final StreamZipper<A> p) {
            return p.next().map(join(P.<StreamZipper<A>, StreamZipper<A>>p2()));
          }
        }, this);

    return streamZipper(left, this, right);
  }
   ...
   */
  public <B> StreamZipper<B> cobind(final F<StreamZipper<A>, B> f) {
   ...
   */
  public StreamZipper<P2<A, Boolean>> zipWithFocus() {
    return streamZipper(left.zip(repeat(false)), P.p(focus, true), right.zip(repeat(false)));
  }
   ...
   */
  public Option<StreamZipper<A>> move(final int n) {
    final int ll = left.length();
    final int rl = right.length();
    Option<StreamZipper<A>> p = some(this);
    if (n < 0 || n >= length())
      return none();
    else if (ll >= n)
      for (int i = ll - n; i > 0; i--)
        p = p.bind(StreamZipper.<A>previous_());
    else if (rl >= n)
      for (int i = rl - n; i > 0; i--)
        p = p.bind(StreamZipper.<A>next_());
    return p;
  }
   ...
   */
  public Option<StreamZipper<A>> find(final Equal<A> e, final A a) {
    for (Option<StreamZipper<A>> x = some(this); x.isSome(); x = x.bind(StreamZipper.<A>previous_()))
      if (e.eq(x.some().focus(), a))
        return x;
    for (Option<StreamZipper<A>> x = some(this); x.isSome(); x = x.bind(StreamZipper.<A>next_()))
      if (e.eq(x.some().focus(), a))
        return x;
    return none();
  }
